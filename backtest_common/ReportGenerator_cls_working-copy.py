...
class ReportGenerator:
    ...
    def generate_signal_level_reports(self, strategy_obj):
        ...
        metrics_table = pd.DataFrame({
            "Metric":[
                ...
                "Average Trade PnL","Average Win","Average Loss","Max Drawdown","Expectancy"
            ],
            "Value":[
                ...
                round(avg_loss,2) if not np.isnan(avg_loss) else np.nan, round(max_drawdown,2), round(expectancy,2)
            ]
        })
    ...
    def aggregate_pnl_reports(self):
        # Automatically scan folder for all *_trade_dashboard_final.xlsx files

        files = glob.glob(os.path.join(self.backtest_reports_dir, "*_trade_dashboard_final.xlsx"))

        if not files:
            raise FileNotFoundError("No files named *_trade_dashboard_final.xlsx found in the folder")

        # Collect DailyPnL sheets from each strategy
        dfs = {}
        for f in files:
            base = os.path.basename(f)
            # Extract strategy_id (everything before _trade_dashboard_final.xlsx)
        ...
        # ByMonth aggregation (Total + per-strategy)
        ...
        # Per-strategy ByMonth sheets
        by_month_per_strategy = {}
        for sid in dfs.keys():
            col = f"{sid}_DailyPnL"
            df_month = by_day.groupby("Month")[[col]].sum().reset_index()
            df_month["Month"] = df_month["Month"].astype(str)
            df_month = df_month.sort_values("Month").reset_index(drop=True)
            by_month_per_strategy[sid] = df_month

        # Save aggregated workbook
        out_path = f"{self.backtest_reports_dir}Aggregated_PnL_with_Charts.xlsx"
        with pd.ExcelWriter(out_path, engine="xlsxwriter") as writer:
            ...
            for sid, df in dfs.items():
                ...
                df_reset.to_excel(writer, sheet_name=f"{sid}_DailyPnL", index=False)
            for sid, df_month in by_month_per_strategy.items():
                df_month.to_excel(writer, sheet_name=f"{sid}_ByMonth", index=False)

            workbook = writer.book

            # Freeze header rows in ByDay and ByMonth
            writer.sheets["ByDay"].freeze_panes(1, 0)
            writer.sheets["ByMonth"].freeze_panes(1, 0)

            # Auto-adjust column widths for all sheets and apply date formatting
            date_fmt = workbook.add_format({"num_format": "yyyy-mm-dd"})
            for sheet_name, worksheet in writer.sheets.items():
                if sheet_name in ["ByDay", "ByMonth"] or sheet_name.endswith("DailyPnL") or sheet_name.endswith("ByMonth"):
                    if sheet_name == "ByDay":
                        df_example = by_day
                    elif sheet_name == "ByMonth":
                        df_example = by_month
                    elif sheet_name.endswith("DailyPnL"):
                        sid = sheet_name.replace("_DailyPnL", "")
                        df_example = dfs[sid].reset_index()
                    elif sheet_name.endswith("ByMonth"):
                    ...
                    if df_example is not None:
                        for idx, col in enumerate(df_example.columns):
                            col_width = max(len(str(col)), df_example[col].astype(str).map(len).max()) + 2
                            if "date" in col.lower() or "Date" in col:
                                worksheet.set_column(idx, idx, col_width, date_fmt)
                            else:
                                worksheet.set_column(idx, idx, col_width)

            # Conditional formatting for PnL columns in ByDay, ByMonth, and per-strategy ByMonth
            ...
            for sheet_name in list(writer.sheets.keys()):
                if sheet_name in ["ByDay", "ByMonth"] or sheet_name.endswith("ByMonth"):
                    ...
                    for idx, col in enumerate(df_example.columns):
                        if "PnL" in col:
                            ws.conditional_format(1, idx, len(df_example), idx, {
            ...
            plt.xticks(rotation=45, ha="right")
            plt.tight_layout()
            chart1_path = f"{self.backtest_reports_dir}dashboard_figs/monthly_pnl.png"
            plt.savefig(chart1_path)
            plt.close()

            # Chart 2: Equity curves (per strategy + total)
            plt.figure(figsize=(10,5))
            for sid in dfs.keys():
                col = f"{sid}_DailyPnL"
                if col in by_day:
                    plt.plot(by_day["TradeDate"], by_day[col].cumsum(), label=sid)
            plt.plot(by_day["TradeDate"], by_day["TotalPnL"].cumsum(), label="Total", linewidth=2, color="black")
            plt.title("Equity Curve (Cumulative PnL)")
            plt.xlabel("Date")
            plt.ylabel("Cumulative PnL")
            ...
            plt.legend()
            plt.tight_layout()
            chart2_path = f"{self.backtest_reports_dir}dashboard_figs/equity_curve.png"
            plt.savefig(chart2_path)
            plt.close()

            # Chart 3: Stacked bar of monthly PnL by strategy
            plt.figure(figsize=(10,6))
            strategy_cols = [c for c in by_month.columns if c.endswith("DailyPnL")]
            by_month.set_index("Month")[strategy_cols].plot(kind="bar", stacked=True, figsize=(10,6))
            plt.title("Monthly PnL by Strategy (Stacked)")
            plt.xlabel("Month")
            plt.ylabel("PnL")
            plt.xticks(rotation=45, ha="right")
            plt.tight_layout()
            chart3_path = f"{self.backtest_reports_dir}dashboard_figs/monthly_pnl_stacked.png"
            plt.savefig(chart3_path)
            plt.close()

            # Insert charts into Excel
            charts_ws.insert_image("B2", chart1_path)
            charts_ws.insert_image("B25", chart2_path)
            charts_ws.insert_image("B48", chart3_path)

            # Add summary table to Charts sheet
            table_start_row = 70
            table_start_col = 1
            summary_headers = list(by_month.columns)

            # Add Grand Total row
            grand_total = ["Grand Total"] + [by_month[col].sum() if col != "Month" else "" for col in summary_headers[1:]]
            summary_data = [summary_headers] + by_month.values.tolist() + [grand_total]

            for r, row_data in enumerate(summary_data):
                for c, val in enumerate(row_data):
                    charts_ws.write(r + table_start_row, c + table_start_col, val)

            # Apply formatting
            header_fmt = workbook.add_format({"bold": True, "bg_color": "#D9E1F2"})
            num_fmt = workbook.add_format({"num_format": "#,##0"})
            pos_fmt2 = workbook.add_format({"num_format": "#,##0", "font_color": "green"})
            neg_fmt2 = workbook.add_format({"num_format": "#,##0", "font_color": "red"})
            total_fmt = workbook.add_format({"bold": True, "bg_color": "#FFF2CC"})

            # Format headers
            charts_ws.set_row(table_start_row, None, header_fmt)

            # Format data cells
            for r in range(1, len(summary_data)):
                for c in range(1, len(summary_headers)):  # skip Month col
                    val = summary_data[r][c]
                    if isinstance(val, (int, float)):
                        fmt = pos_fmt2 if val >= 0 else neg_fmt2
                        if r == len(summary_data) - 1:  # last row = Grand Total
                            fmt = total_fmt
                        charts_ws.write(r + table_start_row, c + table_start_col, val, fmt)
                if r == len(summary_data) - 1:
                    charts_ws.write(r + table_start_row, table_start_col, "Grand Total", total_fmt)

        print("Saved aggregated workbook:", out_path)
