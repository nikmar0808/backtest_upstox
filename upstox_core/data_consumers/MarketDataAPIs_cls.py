...
# to marketfeed_protobuf for V2 APIs
from marketfeed_protobuf_v3 import MarketDataFeed_pb2 as pb

# Create and configure logger
market_data_apis_logger = logging.getLogger("logs/market_data_apis.log")
market_data_apis_logger.addHandler(logging.FileHandler("logs/market_data_apis.log", mode='a'))
...
class MarketDataAPIs:
    ...
    async def fetch_market_data_feed(self, instrument_key):
        ...
        async with websockets.connect(market_data_feed_authorize.data.authorized_redirect_uri, ssl=ssl_context) as websocket:
            ...
            data = {
                "guid": "someguid",#Generate Unique across system
                "method": "sub",
                "data": {
                    "mode": "full",
            ...
            feed_count = 0
            with open(__backtestconfig__.market_data_text_file, "w") as file_text, open(__backtestconfig__.market_data_json_file, "w") as file_json:
