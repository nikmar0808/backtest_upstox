...
from backtest_common.TALibrary_cls import TALibrary as tal
...
class SignalDataCreator:
    ...
    def create_signal_data_for_strategy(self, strategy_obj):
        ...
        if strategy_id in ['NIFTY_S01', 'NIFTY_S02', 'NIFTY_S03', 'NIFTY_S04', 'NIFTY_S05', 'NIFTY_S06', 'NIFTY_S07', 'NIFTY_S08', 'NIFTY_S09', 'NIFTY_S10', 'BANKNIFTY_S05', 'BANKNIFTY_S06']:
            ...
            self.df_hist_data.loc[condition_trade, 'lot_size'] = df_buy_strike_contract_details['lot_size'].astype('Int64')
            self.df_hist_data['lot_size'] = self.df_hist_data['lot_size'].fillna(0).astype('Int64')
            self.df_hist_data.loc[condition_trade, 'freeze_quantity'] = df_buy_strike_contract_details['freeze_quantity'].astype('Int64')
            self.df_hist_data['freeze_quantity'] = self.df_hist_data['freeze_quantity'].fillna(0).astype('Int64')
    ...
    def calculate_indicators(self, strategy_obj):
        ...
        if strategy_id in ['NIFTY_S01']:
            ...
            filter_ema_high = strategy_obj.__dict__.get('entry_filter_ema_high')
            filter_ema_low = strategy_obj.__dict__.get('entry_filter_ema_low')
            ...
            df_ema = tal.calc_ema(self, self.df_hist_data[filter_ema_source], length=filter_ema_length)
            self.df_hist_data['ema_val'] = df_ema['ema'].astype(float)
            df_ema_high = tal.calc_ema(self, self.df_hist_data[filter_ema_high], length=filter_ema_length)
            self.df_hist_data['ema_high_val'] = df_ema_high['ema'].astype(float)
            df_ema_low = tal.calc_ema(self, self.df_hist_data[filter_ema_low], length=filter_ema_length)
            self.df_hist_data['ema_low_val'] = df_ema_low['ema'].astype(float)

            df_rma = tal.calc_rma(self, self.df_hist_data['ema_val'], length=filter_rma_ema_length)
            self.df_hist_data['rma_ema_val'] = df_rma['rma'].astype(float)
            df_rma_high = tal.calc_rma(self, self.df_hist_data['ema_high_val'], length=filter_rma_ema_length)
            self.df_hist_data['rma_ema_high_val'] = df_rma_high['rma'].astype(float)
            df_rma_low = tal.calc_rma(self, self.df_hist_data['ema_low_val'], length=filter_rma_ema_length)
            self.df_hist_data['rma_ema_low_val'] = df_rma_low['rma'].astype(float)

        elif strategy_id in ['NIFTY_S02']:
            ...
            #self.df_hist_data['st_val'] = (tal.supertrend(self.df_hist_data['high'], self.df_hist_data['low'], self.df_hist_data['close'], length=filter_st_atr_length, multiplier=filter_st_atr_factor)[f'SUPERT_{filter_st_atr_length}_{filter_st_atr_factor}']).astype(float)

            #self.df_hist_data['atr_val'] = tal.atr(self.df_hist_data['high'], self.df_hist_data['low'], self.df_hist_data['close'], length=exit_atr_length, mamode='rma').astype(float)
            ...
            #self.df_hist_data['rma_ema_val'] = (tal.rma(self.df_hist_data['ema_val'], length=filter_rma_ema_length)).astype(float)
            df_rma = tal.calc_rma(self, self.df_hist_data['ema_val'], length=filter_rma_ema_length)
            self.df_hist_data.insert(loc=close_col_index + 1, column='rma_ema_val', value=df_rma.astype(float))

            filter_ema = tal.calc_ema(self, self.df_hist_data[filter_ema_source], length=filter_ema_length)
            self.df_hist_data.insert(loc=close_col_index + 2, column='filter_ema_val', value=filter_ema.astype(float))
            ...
            self.df_hist_data['ema_val'] = (tal.ema(self.df_hist_data[filter_ema_source], length=filter_ema_length)).astype(float)

            #self.df_hist_data['rma_ema_val'] = (tal.rma(self.df_hist_data['ema_val'], length=filter_rma_ema_length)).astype(float)
            df_rma = tal.calc_rma(self, self.df_hist_data['ema_val'], length=filter_rma_ema_length)
            self.df_hist_data.insert(loc=close_col_index + 1, column='rma_ema_val', value=df_rma.astype(float))

            df_filter_ema = tal.calc_ema(self, self.df_hist_data[filter_ema_source], length=filter_ema_length)
            self.df_hist_data.insert(loc=close_col_index + 2, column='filter_ema_val', value=df_filter_ema.astype(float))
            ...
            df_st = tal.calc_supertrend(self, self.df_hist_data['high'], self.df_hist_data['low'], self.df_hist_data['close'], length=st_atr_length, multiplier=st_atr_factor)
            self.df_hist_data['st_val'] = df_st['st_val'].astype(float)
            self.df_hist_data['st_dir'] = df_st['st_dir'].astype(float)
            self.df_hist_data['st_lb'] = df_st['st_lb'].astype(float)
            self.df_hist_data['st_ub'] = df_st['st_ub'].astype(float)

            df_filter_ema = tal.calc_ema(self, self.df_hist_data[filter_ema_source], length=filter_ema_length)
            self.df_hist_data['ema_val'] = df_filter_ema.astype(float)

            df_rma = tal.calc_rma(self, self.df_hist_data['ema_val'], length=filter_rma_ema_length)
            self.df_hist_data['rma_ema_val'] = df_rma.astype(float)
    ...
    def identify_entry_signal_rows(self, strategy_obj):
        ...
        if strategy_id in ['NIFTY_S01']:
            ...
            condition_entry = (
                                (self.df_hist_data['close'].shift(3) <= self.df_hist_data['first_candle_high_extended'].shift(3)) & \
                                (self.df_hist_data['close'].shift(2) > self.df_hist_data['first_candle_high_extended'].shift(2))
                            ) & \
                            (self.df_hist_data['close'].shift(1) < self.df_hist_data['open'].shift(1)) & \
            ...
            if strategy_entry_filter_ema:
                ...
                #                condition_entry
                condition_filter = ~ ((self.df_hist_data['rma_ema_low_val'] < self.df_hist_data['open']) & \
                                (self.df_hist_data['open'] < self.df_hist_data['rma_ema_high_val'])) & \
                                condition_entry
            else:
                condition_filter = condition_entry
            ...
            # and current candle open time >= strategy entry_time and < strategy entry_cut_off
            condition_entry = (
                                (self.df_hist_data['close'].shift(3) >= self.df_hist_data['first_candle_low_extended'].shift(3)) & \
    ...
    def set_trade_instr_details(self, strategy_obj):
        ...
        if strategy_id in ['NIFTY_S01', 'NIFTY_S02', 'NIFTY_S03', 'NIFTY_S04', 'NIFTY_S05', 'NIFTY_S06', 'BANKNIFTY_S05', 'BANKNIFTY_S06']:
        # elif strategy_id in ['NIFTY_S03', 'NIFTY_S04', 'NIFTY_S05', 'NIFTY_S06', 'BANKNIFTY_S05', 'BANKNIFTY_S06']:

            strike_increment = strategy_obj.__dict__.get('strike_increment')
            buy_strike_offset = strategy_obj.__dict__.get('buy_strike_offset')
            buy_strike_right = strategy_obj.__dict__.get('buy_strike_right')

            condition_trade = self.df_hist_data['is_entry_filter']
            filtered_open = self.df_hist_data.loc[condition_trade, 'open']
            self.df_hist_data.loc[condition_trade, 'buy_strike'] = (self.get_buy_strike(filtered_open, strike_increment, buy_strike_offset, buy_strike_right)).astype('Int64')
            self.df_hist_data['buy_strike'] = self.df_hist_data['buy_strike'].fillna(0).astype('Int64')
    ...
    def get_buy_strike(self, open_series, strike_increment, buy_strike_offset, buy_strike_right) -> pd.Series:
        strike_series = pd.Series(index=open_series.index, dtype=int)

        if buy_strike_right == "PE":
            remainder = open_series % strike_increment
            zero_rem_indices = remainder[remainder == 0].index
            nozero_rem_indices = remainder[remainder != 0].index
            strike_series.loc[zero_rem_indices] = pd.Series((open_series[zero_rem_indices] + strike_increment + buy_strike_offset).astype('Int64'))
            strike_series.loc[nozero_rem_indices] = pd.Series(((open_series[nozero_rem_indices] // strike_increment + 1) * strike_increment + buy_strike_offset).astype('Int64'))
            ...
            nozero_rem_indices = remainder[remainder != 0].index
            strike_series.loc[zero_rem_indices] = pd.Series((open_series[zero_rem_indices] - strike_increment - buy_strike_offset).astype('Int64'))
            strike_series.loc[nozero_rem_indices] = pd.Series(((open_series[nozero_rem_indices] // strike_increment) * strike_increment - buy_strike_offset).astype('Int64'))
            strike_series.sort_index()
    ...
    def get_buy_strike_expiry_details(self, candle_sttime_series, strategy_id) -> pd.DataFrame:
        ...
        for i, val in expiry_series[exp_data_avl_indices].items():
            ...
            if not greater_than_val.empty:
                ...
                if strategy_id in ['NIFTY_S01', 'NIFTY_S02', 'NIFTY_S03', 'NIFTY_S04', 'NIFTY_S05', 'NIFTY_S06', 'NIFTY_S07', 'NIFTY_S08', 'NIFTY_S09', 'NIFTY_S10']:
                # elif strategy_id in ['NIFTY_S03', 'NIFTY_S04', 'NIFTY_S05', 'NIFTY_S06', 'NIFTY_S07', 'NIFTY_S08', 'NIFTY_S09', 'NIFTY_S10']:
    ...
    def update_buy_strike_contracts_details(self, signal_buy_strike_series, strategy_obj) -> pd.DataFrame:
        ...
        if strategy_id in ['NIFTY_S01', 'NIFTY_S02', 'NIFTY_S03', 'NIFTY_S04', 'NIFTY_S05', 'NIFTY_S06', 'NIFTY_S07', 'NIFTY_S08', 'NIFTY_S09', 'NIFTY_S10', 'BANKNIFTY_S05', 'BANKNIFTY_S06']:
            ...
            for i, row in df_signal_file_trade.iterrows():
                ...
                # Check if the filtered DataFrame actually contains rows
                if not df_contract_file_filtered.empty:
                    ...
                    inst_key_series[i] = None  # Or np.nan, or a default string                
                    trading_symbol_series[i] = None  # Or np.nan, or a default string
                    lot_size_series[i] = None  # Or np.nan, or a default string
                    freeze_quantity_series[i] = None  # Or np.nan, or a default string
                # inst_key_series[i] = df_contract_file_filtered['instrument_key'].values[0]
                # trading_symbol_series[i] = df_contract_file_filtered['trading_symbol'].values[0]
                # lot_size_series[i] = df_contract_file_filtered['lot_size'].values[0]
                # freeze_quantity_series[i] = df_contract_file_filtered['freeze_quantity'].values[0]
